...
[[noreturn]] void ObjFailAndExit(const char* message) {
    std::cout << "OBJ parser error: " << message << std::endl;
    std::exit(-1);
}

int ResolveObjIndex(const int rawIndex, const size_t count) {
    if (rawIndex > 0) {
        return rawIndex - 1;
    }
    if (rawIndex < 0) {
...
ObjIndexTriplet ParseObjFaceVertexToken(const std::string& token) {
...
uint32_t GetOrCreateObjVertexIndex(
...
void CenterModel(ObjModel& model, const bool normalize) {
    float maxF = 1e30f;
...
void LoadTexture(RenderingState& state, const std::filesystem::path& texturePath,
...
void LoadTexture(RenderingState& state, const wchar_t* path, ComPtr<ID3D11ShaderResourceView>& texture) {
    const std::filesystem::path filePath(path);
    LoadTexture(state, filePath, texture);
}

ObjModel LoadObjModel(RenderingState& state, const wchar_t* folder, const bool doCentering) {
    if (folder == nullptr) {
...
RawMesh CreateSphere(const float size) {
    RawMesh mesh;
...
RawMesh CreateCube(const float sizeX, const float sizeY, const float sizeZ) {
    RawMesh mesh;
...
RawMesh CreateSpotLightBoundingBox(const float range, const float spotOuterAngle) {
